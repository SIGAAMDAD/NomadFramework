...
namespace Nomad.EngineUtils.Godot.Private.SceneManagement {
	...
	*/
	...
	internal sealed class GodotSceneManager : ISceneManager {
		...
		private readonly List<GodotGameObject> _objectList = new();
		...
		public IScene LoadScene( string path, LoadSceneMode mode = LoadSceneMode.Single ) {
			...
			switch ( mode ) {
				case LoadSceneMode.Additive:
					_sceneTree.Root.AddChild( ( scene.Root as GodotGameObject ).Node );
		...
		public void SetActiveScene( IScene scene ) {
			...
			UnloadAllScenes();
			_sceneTree.CallDeferred( SceneTree.MethodName.ChangeSceneToNode, ( godotScene.Root as GodotGameObject ).Node );
		...
		public void UnloadScene( IScene scene ) {
			...
			if ( _additiveScenes.Contains( godotScene ) ) {
				_sceneTree.Root.CallDeferred( Node.MethodName.RemoveChild, ( godotScene.Root as GodotGameObject ).Node );
			}
		...
		public void UnloadAllScenes() {
			UnloadCurrentScene();
			for ( int i = 0; i < _additiveScenes.Count; i++ ) {
				_sceneTree.Root.CallDeferred( Node.MethodName.RemoveChild, ( _additiveScenes[i].Root as GodotGameObject ).Node );
				_sceneCache.Unload( _additiveScenes[i].Path );
			}
		...
		public void UnloadCurrentScene() {
			...
			_sceneTree.Root.CallDeferred( Node.MethodName.RemoveChild, ( _activeScene.Root as GodotGameObject ).Node );
			_sceneCache.Unload( _activeScene.Path );
			_activeScene = null;
		}
