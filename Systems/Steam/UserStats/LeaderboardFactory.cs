...
namespace Steam.UserStats {
	public static class LeaderboardFactory {
		...
		private static SteamLeaderboardEntries_t LeaderboardEntriesHandle;
		...
		===============
		*/
		public static async Task<Leaderboard> GetLeaderboardData( string? leaderboardName ) {
			ArgumentException.ThrowIfNullOrEmpty( leaderboardName );

			SteamAPICall_t handle = SteamUserStats.FindLeaderboard( leaderboardName );
			OnLeaderboardFindResult.Set( handle );

			lock ( LeaderboardFound ) {
				Monitor.Wait( LeaderboardFound );
			}
			return null;
		}
		...
		*/
		private static void OnFindLeaderboard( LeaderboardFindResult_t pCallback, bool bIOFailure ) {
			if ( pCallback.m_bLeaderboardFound == 0 ) {
				lock ( LeaderboardFound ) {
					Monitor.Pulse( LeaderboardFound );
				}
				return;
			}
