/*
===========================================================================
The Nomad AGPL Source Code
Copyright (C) 2025 Noah Van Til

The Nomad Source Code is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as published
by the Free Software Foundation, either version 3 of the License, or
(at your option) any later version.

The Nomad Source Code is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with The Nomad Source Code.  If not, see <http://www.gnu.org/licenses/>.

If you have questions concerning this license or the applicable additional
terms, you may contact me via email at nyvantil@gmail.com.
===========================================================================
*/

using NomadCore.Enums.EventSystem;
using NomadCore.Interfaces.EventSystem;
using System;
using System.Threading;
using System.Threading.Tasks;

namespace NomadCore.Systems.EventSystem.Common {
	/*
	===================================================================================
	
	StaticGameEvent
	
	===================================================================================
	*/
	/// <summary>
	/// 
	/// </summary>
	
	public class StaticGameEvent<TArgs> : IGameEvent<TArgs> where TArgs : IEventArgs {
		public string? Name => throw new NotImplementedException();

		public EventThreadingPolicy ThreadingPolicy => throw new NotImplementedException();

		public void Dispose() {
			throw new NotImplementedException();
		}

		public EventThreadingPolicy GetDefaultThreadingPolicy() {
			throw new NotImplementedException();
		}

		public void Publish( TArgs eventArgs ) {
			throw new NotImplementedException();
		}

		public void Publish( IEventArgs eventArgs ) {
			throw new NotImplementedException();
		}

		public Task PublishAsync( TArgs eventArgs, CancellationToken cancellationToken = default ) {
			throw new NotImplementedException();
		}

		public void Subscribe( object? subscriber, Func<TArgs, CancellationToken, Task> asyncCallback ) {
			throw new NotImplementedException();
		}

		public void Subscribe( object? subscriber, IGameEvent<TArgs>.GenericEventCallback? callback ) {
			throw new NotImplementedException();
		}

		public void Subscribe( object? subscriber, IGameEvent.EventCallback? callback ) {
			throw new NotImplementedException();
		}

		public void Unsubscribe( object? subscriber, IGameEvent<TArgs>.GenericEventCallback? callback ) {
			throw new NotImplementedException();
		}

		public void Unsubscribe( object? subscriber, IGameEvent.EventCallback? callback ) {
			throw new NotImplementedException();
		}
	};
};