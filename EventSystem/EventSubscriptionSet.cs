	...
	*/
	...
	public sealed class EventSubscriptionSet : IDisposable {
		...
		*/
		...
		public void AddFriend( in IGameEvent? otherEvent ) {
		...
		public void Pump( IEventArgs args, bool singleThreaded ) {
			lock ( Lock ) {
				...
				if ( singleThreaded ) {
					...
					System.Threading.Tasks.Parallel.ForAsync( 0, Subscriptions.Count, async ( index, value ) => await PumpEvent( index, args ) );
				}
			}
		}
		...
		*/
		...
		/// <param name="args"></param>
		/// <returns></returns>
		private System.Threading.Tasks.ValueTask PumpEvent( int index, IEventArgs args ) {
